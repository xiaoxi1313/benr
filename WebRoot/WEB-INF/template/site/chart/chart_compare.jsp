...
z-index:9999999;
  ...
  function addrow() {
		...
		$(o).removeAttr("id");
		$(o).removeAttr("style");
		$(o).find('select[c="ccc"]').removeAttr('id');
		$(o).find('select[c="ccc"]').removeAttr('style');
		$(o).find('select[c="ccc"]').attr('name','cid_'+idnum);
		$(o).find('select[a="aaa"]').attr('name','fid_'+idnum);
		
		$(o).find("input[name='optionsRadios']").attr("name","optionsRadios"+idnum);
		$(o).find("input[name='tagNames']").attr("name","tagNames"+idnum);
		$(o).find("input[name='beginDates']").attr("name","beginDates"+idnum);
		$(o).find("input[name='endDates']").attr("name","endDates"+idnum);
		...
		function (data, textStatus) {
			if (data.errorCode == 0) {
				...
				$.each(data.favoriteList, function(i, item){
					$("<option value=\"" + item.id + "\">" + item.name + "</option>").appendTo($(o));
				});
...
function cauto() {
	$('.ac1').each(function(){
		var ob = this;
		$(ob).autocomplete('tag_listAllTag.action', {
			minChars: 1,
	        remoteDataType: 'json',
    ...
    $("input[name='selectidx']").click(function(){
    	if(this.checked) {
...
function tochart() {
	$.post('chart_compare.action',$('#chartForm').serialize(),function(d){
		if(d.errorCode==0){
			$('#container').show();
...
function selectsource(obj) {
	...
	$(obj).parent().parent().parent().find('input[name="optionsRadios"]').each(function(){
            ...
            <section class="scrollable padder">
              ...
              <section class="panel panel-default">
                  ...
                  <thead>
	                  ...
	                  <th width="30">
	                  <label class="checkbox m-t-none m-b-none i-checks" style="margin-left: 6px;">
	                  	<input type="checkbox" name="selectidx" value="0" checked="checked"><i style="margin-left: 0px;"></i></label></th>
	                  ...
	                  <div class="radio">
	                   ...
	                   <div class="row wrapper selectrow selectr2">
	                    ...
	                    <div class="btn-group m-r input-group date form_date" data-date="" data-date-format="" data-link-field="" data-link-format="yyyy-mm-dd">
	                     <input class="form-control inputdate" size="16" type="text" value="" readonly=""  name="beginDates">
	                     <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
	                    	<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
	                     </div>
	                    <div class="btn-group m-r input-group date form_date" data-date="" data-date-format="" data-link-field="" data-link-format="yyyy-mm-dd">
	                     <input class="form-control inputdate" size="16" type="text" value="" readonly="" name="endDates">
	                     <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
	                    	<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
	                     </div>
	                    </div>
                  ...
                  <a href="javascript:void(0);" class="btn btn-s-md btn-primary" id="btnalc" onclick="chart()">开始计算</a>
