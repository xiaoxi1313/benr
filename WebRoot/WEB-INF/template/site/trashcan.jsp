...
// 物理删除
function physicsDelete(id) {
	if(confirm("确定永久删除当前记录?")) {
		$.post("article_physicsDelete.action?article.id=" + id, 
			      function(data){
			     	if(data.errorCode==0){
			     		alert("操作成功！");
			     		location.reload();
...
// 移出回收站
function removeRecycle(id) {
	$.post("article_removeRecycle.action?article.id=" + id, 
	      function(data){
	     	if(data.errorCode==0){
	     		alert("操作成功！");
	     		location.reload();
            ...
            <section>
              <section class="vbox">
                <section class="scrollable padder">              
                  ...
                  <div class="row">
                    <div class="col-sm-12">
                      <h3 class="m-b-xs text-black">回收站</h3>
                      <small>您可以在此查看不需要的资讯，也可以彻底删除资讯。</small>
                    </div>
                    </div>
                  </section>
                      ...
                      <button data-toggle="dropdown" class="btn btn-sm btn-default dropdown-toggle">
                                    ...
                                    <form id="searchForm" method="post" action="article_list.action">
                                      <div class="input-group">
                                        <input type="text" name="article.title" value="<s:property value='article.title' />" class="input-sm form-control" placeholder="搜索">
                                  ...
                                  <form id="batchOperateForm" method="post" action="article_batchPhysicsDelete.action">
                                  <!-- 列表 -->
                                    ...
                                    <table class="table table-striped b-t b-light">
                                      <thead>
                                        <tr>
                                          ...
                                          <th class="th-sortable" data-toggle="class" style="width:70px;" orderfield="id" orderCond="asc">ID
                                            <span class="th-sort">
                                          ...
                                          <th>标题</th>
                                          <th class="th-sortable" data-toggle="class" orderfield="createTime" orderCond="asc">修改时间
                                          <span class="th-sort">
                                            <i class="fa fa-sort-down text"></i>
                                            <i class="fa fa-sort-up text-active"></i>
                                            <i class="fa fa-sort"></i>
                                      ...
                                      <tbody>
                                      	<s:iterator value="articleList" id="article" status="u">
	                                          ...
	                                          <td><div class="btn-group open">
	                                              <a href="javascript:void(0)" onclick="removeRecycle(<s:property value='id'/>)" class="iconbtn" data-toggle="tooltip" data-placement="top" title="" data-original-title="移出回收站"><i class="fa fa-share-square-o"></i></a>
	                                              <a href="javascript:void(0)" onclick="physicsDelete(<s:property value='id'/>)" class="iconbtn" data-toggle="tooltip" data-placement="top" title="" data-original-title="彻底删除"><i class="fa fa-trash-o"></i></a></div>
	                                              </td>
	                                        </tr>
                                      	</s:iterator>
